...
class LoginAttemptManager {
  ...
  getAttemptInfo(ip, phone) {
    ...
    // 檢查是否過期或已解鎖
    if (now.diff(attempt.firstAttempt, 'hour') >= 24) {
    ...
    // 如果已解鎖但未過期，保留嘗試次數
    if (attempt.lockUntil && now.isAfter(attempt.lockUntil)) {
  ...
  recordFailedAttempt(ip, phone) {
    ...
    if (attempt.count >= 7) {
      ...
      const lockMinutes = attempt.count === 5 ? 2 : 5;
...
export default new LoginAttemptManager();
